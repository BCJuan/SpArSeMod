...
def deconv_flops_counter_hook(conv_module, input, output):
    ...
    conv_module.__maxram__ = (
        np.prod(input.shape[1:])
        + np.prod(output.shape[1:])
        + np.prod(conv_module.weight.shape)
    )
...
def add_flops_counter_variable_or_reset(module):
    if is_supported_instance(module):
        if (
            hasattr(module, "__flops__")
            or hasattr(module, "__params__")
            or hasattr(module, "__maxram__")
        ):
            print(
                "Warning: variables __flops__ , __params__ or __maxram__ are already "
